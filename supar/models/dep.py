...
from supar.structs import (CRF2oDependency, CRFDependency, LBPDependency,
                           MatrixTree, MFVIDependency)
...
class BiaffineDependencyModel(Model):
    ...
    def decode(self, s_arc, s_rel, mask, tree=False, proj=False):
        ...
        if tree and any(bad):
            arc_preds[bad] = (CRFDependency if proj else MatrixTree)(s_arc[bad], mask[bad]).argmax
...
class CRF2oDependencyModel(BiaffineDependencyModel):
    ...
    def decode(self, s_arc, s_sib, s_rel, mask, tree=False, mbr=True, proj=False):
        ...
        if tree and any(bad):
            if proj and not mbr:
                arc_preds[bad] = CRF2oDependency((s_arc[bad], s_sib[bad]), mask[bad]).argmax
            else:
                arc_preds[bad] = (CRFDependency if proj else MatrixTree)(s_arc[bad], mask[bad]).argmax
        rel_preds = s_rel.argmax(-1).gather(-1, arc_preds.unsqueeze(-1)).squeeze(-1)
